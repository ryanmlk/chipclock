...
type JobType = 'grill' | 'tortilla' | 'line' | 'prep' | 'closing prep' | 'closing salsa' | 'closing dml' | 'cash';
...
const ChipotleAvailabilityTracker: React.FC = () => {
  ...
  const [stats, setStats] = useState<Stats>({ totalEmployees: 0, availableToday: 0, bestCoveredDay: '-' });
  ...
  const quickEditEmployee = async (id: number, day: DayOfWeek, value: string) => {
    try {
  ...
  const calculateDaysHours = (schedule: Omit<FormData, 'name' | 'email' | 'jobType' | 'effectiveDate'>): string => {
    const days: string[] = [
      schedule.monday, 
      schedule.tuesday, 
      schedule.wednesday, 
    ...
    const workingDays = days.filter(day => day && day !== 'N/A' && day.trim() !== '').length;
  ...
  const handleQuickEdit = async (employeeId: number, day: DayOfWeek): Promise<void> => {
    const employee = employees.find(emp => emp.id === employeeId);
    if (!employee) return;

    const newValue = window.prompt(`Enter availability for ${day}:`, employee[day] || '');
    if (newValue !== null) {
    ...
    <div className="min-h-screen bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gray-800 text-white p-6 rounded-t-lg">
          <h1 className="text-2xl font-bold text-center">Chipotle Schedule</h1>
        </div>

        {/* Controls */}
        <div className="bg-gray-800 p-6 border-x border-gray-700 flex flex-wrap gap-4 items-end">
          <div className="space-y-2">
            <Label className="text-white">Filter by Job Type</Label>
            <Select value={jobFilter} onValueChange={(value: JobType | 'all') => setJobFilter(value)}>
          ...
          <div className="space-y-2">
            <Label className="text-white">Search Employee</Label>
            <Input
              placeholder="Search employee name..."
              value={employeeFilter}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEmployeeFilter(e.target.value)}
              className="w-48"
            />
          </div>

          <div className="space-y-2">
            <Label className="text-white">Week Starting</Label>
            <Input
              type="date"
              value={weekStart}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setWeekStart(e.target.value)}
              className="w-48"
            />
          </div>
        ...
        <div className="bg-gray-800 p-6 border-x border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-gray-700 border-gray-600">
            <CardHeader className="pb-2">
              <CardTitle className="text-2xl font-bold text-white">{stats.totalEmployees}</CardTitle>
              <CardDescription className="text-gray-300">Total Employees</CardDescription>
            </CardHeader>
          </Card>
          <Card className="bg-gray-700 border-gray-600">
            <CardHeader className="pb-2">
              <CardTitle className="text-2xl font-bold text-white">{stats.availableToday}</CardTitle>
              <CardDescription className="text-gray-300">Available Today</CardDescription>
            </CardHeader>
          </Card>
          <Card className="bg-gray-700 border-gray-600">
            <CardHeader className="pb-2">
              <CardTitle className="text-2xl font-bold text-white">{stats.bestCoveredDay}</CardTitle>
              <CardDescription className="text-gray-300">Best Covered Day</CardDescription>
            </CardHeader>
          </Card>
        </div>

        {/* Table */}
        <div className="bg-gray-800 rounded-b-lg border border-gray-700 overflow-hidden">
          <div className="overflow-x-auto max-h-96 overflow-y-auto">
            <table className="w-full border-collapse">
              <thead className="sticky top-0 bg-gray-700">
                <tr>
                  <th className="text-white font-bold p-3 border border-gray-600 text-left">Employee Name</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Job Type</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Monday</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Tuesday</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Wednesday</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Thursday</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Friday</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Saturday</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Sunday</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Days/Hr</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Effective Date</th>
                  <th className="text-white font-bold p-3 border border-gray-600 text-center">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredEmployees.map((employee) => (
                  <tr key={employee.id} className="hover:bg-gray-700">
                    <td className="font-medium sticky left-0 bg-gray-800 p-3 border border-gray-600 text-white">
                      {employee.name}
                    </td>
                    <td className="p-3 border border-gray-600 text-center">
                      <Badge className={`${getJobTypeColor(employee.job_type)} text-white`}>
                        {employee.job_type.toUpperCase()}
                      </Badge>
                    </td>
                    <td 
                      className="cursor-pointer hover:bg-gray-600 p-3 border border-gray-600 text-center text-white"
                    ...
                    <td className="p-3 border border-gray-600 text-center text-white">{employee.days_hr}</td>
                    <td className="p-3 border border-gray-600 text-center text-white">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(value: JobType) => handleFormChange('jobType', value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
